...
Our experimental data were collected from the ATLAS particle detector in the Large Hadron Collider (LHC).
...
In order to generate such a high magnetic field, the coils need to have very high current of 11,080 A, and hence superconducting coil need to be used, to reduce the heat loss due to the electrical resistance.
The material of superconducting coil is niobium-titanium (NbTi).
To reach the condition for superconductivity, the electromagnets operate at a very low temperature of 1.9 K.
...
The time-spacing between two adjacent bunches is 25 ns (or 50 ns in the old configuration).
...
The sensor in the module is using planar n$^{+}$-in-n type of silicon with 250 $\mu$m thick, with n$^{+}$-type at the readout side (i.e. the electronics chip) and n-type at another side.
...
TRT has the spatial resolution of 130$\mu$m in the R-$\phi$ plane.
